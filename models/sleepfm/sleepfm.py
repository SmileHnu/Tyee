...
class SleepFM(nn.Module):
    def __init__(self,
                 num_classes,             # 下游分类任务的类别数量
                 bas_in_channels,         # BAS 模态的输入通道数 (通常是 'Sleep_Stages' 的通道数)
                 ecg_in_channels,         # ECG 模态的输入通道数
                 resp_in_channels,        # Respiratory 模态的输入通道数
                 embedding_dim=512,       # 每个 EffNet 编码器输出的嵌入特征维度
                 freeze_encoders=True,    # 是否冻结编码器的参数
                 pretrained_checkpoint_path=None, # 预训练编码器的检查点路径 (可选)
                 # EffNet 编码器的构建参数 (可以使用 EffNet 类中的默认值)
                 effnet_depth=[1,2,2,3,3,3,3],
                 effnet_channels_config=[32,16,24,40,80,112,192,320,1280], # EffNet 内部的阶段输出通道
                 effnet_expansion=6,
                 effnet_stride=2,      # EffNet 第一个卷积层的 stride
                 effnet_dilation=1     # EffNet 第一个卷积层的 dilation
                 ):
        ...
        common_effnet_params = {
            'depth': effnet_depth,
            'channels': effnet_channels_config, # EffNet内部各阶段的通道数
            ...
            'num_additional_features': 0 # EffNet 原本用于拼接额外特征的参数，这里设为0
        }

        # BAS (Brain Activity Signals) 编码器, 通常对应 Sleep_Stages 模态
            ...
            try:
                ...
                def _load_state_dict_flexible(model_to_load, state_dict_from_checkpoint, model_name_for_log):
                    ...
                    for k, v in state_dict_from_checkpoint.items():
                        name = k[7:] if k.startswith('module.') else k # 移除 'module.' 前缀 (如果存在)
                        new_state_dict[name] = v
                    
                    # 加载权重，允许不完全匹配 (例如，如果原始 EffNet 的 fc 层不同，但我们已经替换了它)
                    # 但因为我们在这里已经将 fc 层统一为输出 embedding_dim，理论上应该能匹配
                ...
                # 根据您提供的参考代码中的键名加载:
                # "sleep_state_dict" for BAS/Sleep encoder
                # "ekg_state_dict" for EKG encoder
                # "resp_state_dict" for Respiratory encoder
        ...
        # 3. 根据 freeze_encoders 参数决定是否冻结编码器的参数
        if freeze_encoders:
        ...
        # 4. 定义下游的单线性分类头
        downstream_input_dim = embedding_dim * 3  # 拼接三个模态的嵌入
    ...
    def forward(self, bas_input, ecg_input, resp_input):
        # 输入的信号维度应为 (N, C_modality, L_sequence)
        # N: batch_size, C_modality: 该模态的通道数, L_sequence: 序列长度
