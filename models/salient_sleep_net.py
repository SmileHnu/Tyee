...
class MSEBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int,
        ...
        # Bottleneck convolutions after concatenation
        # Keras code: Conv2D(filter * 2, ...), then Conv2D(filter, ...)
        # Here, out_channels is 'filter' from Keras.
        # Concatenated channels = out_channels * len(dilation_rates)
...
class Branch(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        # Decoder starts from output of encoder 5 (u5_mse) and works upwards with u4_mse, u3_mse etc.
        # Decoder 4, 3, 2, 1
        # Keras: d4 = create_u_encoder(concatenate([up4, u4_mse]), self.filters[3], ...)
        # Input to decoder U-unit: upsampled_prev_decoder_out + corresponding_encoder_mse_out
        # Output channels of decoder U-unit: self.filters[3-i]
        # Output channels of decoder reduce_conv: self.filters[3-i] // 2
        
        # Last MSE output (u5_mse) is the initial input for the decoder part
        # Its channels are self.mse_filters[4]
        ...
        for i in range(4): # 4 decoder stages (d4, d3, d2, d1)
            ...
            # Channels for concat: upsampled channels + skip connection (encoder_mse_out) channels
            # upsampled channels = current_decoder_in_channels_from_prev_stage
            # skip connection channels = self.mse_filters[encoder_stage_idx]
    ...
    def forward(self, x):
        ...
        for i in range(4): # d4, d3, d2, d1
            # Skip connection is from corresponding encoder MSE output
            # For d4 (i=0), skip is u4_mse (mse_outputs[3])
            # For d3 (i=1), skip is u3_mse (mse_outputs[2])
            # ...
            # For d1 (i=3), skip is u1_mse (mse_outputs[0])
        ...
        # Final output of the branch is d1_reduced (decoder_current_x)
        # Keras code has ZeroPadding2D at the end of build_branch
        # Target height is self.sequence_length * self.sleep_epoch_length
        # d1_reduced.size(2) is current height
        
        # The Keras ZeroPadding2D: padding=((int((target_H - current_H) // 2)), 0)
        # This implies symmetric vertical padding, no horizontal padding.
        # PyTorch nn.ZeroPad2d takes (pad_left, pad_right, pad_top, pad_bottom)
        
        # d1_reduced is the output of the last reduce_conv in the decoder.
        # Its channels are self.filters[0] // 2.
        # The Keras code's build_branch returns d1 (output of UEncoder) after ZeroPadding.
        # Let's re-check the Keras build_branch return: it's `zpad(d1)`.
        # `d1` is the output of the last `create_u_encoder` in the decoder.
        # Its channels are `self.filters[0]`.
        # My decoder_current_x is the *reduced* output.
        # The Keras code:
        # up1 = upsample(u1, ...)(d2) -> u1 is u1_mse
        # d1 = create_u_encoder(cat([up1, u1_mse]), self.filters[0], ...)
        # zpad = ZeroPadding2D(...)(d1)
        # So, the last decoder U-unit (my self.decoder_u_units[3]) should output self.filters[0] channels.
        # And there's no reduce_conv after the last decoder U-unit in the Keras path that leads to zpad.
        # My loop for decoder_reduce_convs might be one too many or applied incorrectly for the final d1.

        # Let's adjust the BranchTorch decoder loop for d1
        # The last U-unit (for d1) should not have a reduce_conv after it if its output is directly padded.
        # The output of the branch should have self.filters[0] channels.
        # The last element of self.decoder_reduce_convs is for d1. Its output is self.filters[0]//2. This is not right.
        # The Keras code's d1 (output of create_u_encoder) has self.filters[0] channels.
        
        # Corrected logic: The final output of the branch is the output of the last decoder U-unit.
        # The `decoder_current_x` after the loop is the output of the reduce_conv for d2.
        # Let's trace `d1` in Keras:
        # `up1 = upsample(u1_mse, ...)(d2_reduced)`
        # `d1 = create_u_encoder(cat([up1, u1_mse]), self.filters[0], ...)`
        # So the last `self.decoder_u_units` (index 3) is this `d1`. Its output is directly used for padding.
        # The `decoder_reduce_convs` should only go up to d2.

        # Let's re-do the forward pass for decoder in BranchTorch
        # --- Decoder Path (Corrected) ---
...
class TwoStreamSalientModel(nn.Module):
    ...
    def forward(self, x_eeg, x_eog):
        ...
        # Keras Reshape: (N, seq_len, epoch_len, filters[0]) from (N, seq_len*epoch_len, 1, filters[0])
        # PyTorch from (N, filters[0], seq_len*epoch_len, 1) to (N, filters[0], seq_len, epoch_len)
        # This means H becomes seq_len, W becomes epoch_len
        # Current shape of attention_out: (N, self.filters[0], self.sequence_length * self.sleep_epoch_length, 1)
        
        # Reshape to (N, C, H_seq, W_epoch)
        # C = self.filters[0]
        # H_seq = self.sequence_length
        # W_epoch = self.sleep_epoch_length
        ...
        # Keras: Conv2D(self.filters[0], (1,1), activation='tanh', padding='same')(reshaped_NHWC)
        # PyTorch: nn.Conv2d(C, C, kernel_size=1)
        x = self.final_conv1(reshaped_attention)
        x = self.final_act1(x) # Tanh
        
        # Keras: AveragePooling2D((1, self.sleep_epoch_length))(x_NHWC) -> pools along W_epoch
        # PyTorch: nn.AvgPool2d(kernel_size=(1, W_epoch)) on (N, C, H_seq, W_epoch)
        # Output shape: (N, C, H_seq, 1)
        x = self.final_pool(x)
        
        # Keras: Conv2D(5, (self.kernel_size,1), padding=self.padding, activation='softmax')(x_NHWC_pooled)
        # PyTorch: nn.Conv2d(C, 5, kernel_size=(kernel_H, 1), padding=(pad_H, 0))
        # Output shape: (N, 5, H_seq, 1)
        out_logits = self.final_conv2(x)
        
        # Apply softmax over the channel dimension (dim=1 for NCHW)
        out_probs = self.final_softmax(out_logits) # (N, 5, sequence_length, 1)
        
        # To match a typical output for sequence labeling (N, sequence_length, num_classes)
        # we might want to permute and squeeze
