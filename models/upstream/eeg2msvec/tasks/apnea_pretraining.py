...
from typing import Optional, OrderedDict, TypeVar, Annotated
from fairseq.data.multi_corpus_dataset import MultiCorpusDataset
from omegaconf import MISSING, II

from fairseq.data import BinarizedAudioDataset, SubsampleDataset
from fairseq.dataclass import FairseqDataclass, ChoiceEnum
from fairseq.data.text_compressor import TextCompressionLevel

from fairseq.tasks import FairseqTask, register_task
from ..data.apena_segment_dataset import FileAudioSegmentDataset

CompressionLevelType = TypeVar("CompressionLevelType", bound=TextCompressionLevel)

logger = logging.getLogger(__name__)


@dataclass
class ApenaMaskingConfig:
...
@dataclass
class ApenaPretrainingConfig(FairseqDataclass):
    ...
    # text_compression_level: Annotated[CompressionLevelType, TextCompressionLevel] = field(
    # text_compression_level: str = field(
    ...
    precompute_mask_config: Optional[ApenaMaskingConfig] = None

    post_save_script: Optional[str] = None

    subsample: float = 1
    seed: int = II("common.seed")
    num_per_segment: int = field(
        default=140000,
        metadata={'help': "the segment length"}
    )


@register_task(name="apnea_pretraining", dataclass=ApenaPretrainingConfig)
class ApenaPretrainingTask(FairseqTask):
    cfg: ApenaPretrainingConfig

    @classmethod
    def setup_task(cls, cfg: ApenaPretrainingConfig, **kwargs):
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass = None, **kwargs):
        ...
        if getattr(task_cfg, "binarized_dataset", False):
            ...
            if task_cfg.multi_corpus_keys is None:
                ...
                self.datasets[split] = FileAudioSegmentDataset(
                    ...
                    normalize=task_cfg.normalize,
                    num_buckets=self.cfg.num_batch_buckets or int(self.cfg.tpu),
                    num_per_segment=int(self.cfg.num_per_segment),
                ...
                for key, file_name in data_keys:
                    ...
                    dataset_map[k] = FileAudioSegmentDataset(
                        manifest_path=manifest_path,
                        sample_rate=task_cfg.get("sample_rate", self.cfg.sample_rate),
                        max_sample_size=self.cfg.max_sample_size,
                        min_sample_size=self.cfg.min_sample_size,
